...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle){
	...
	else if(eventIT && bufferIT && I2C_GetFlagStatus(pI2CHandle->pI2Cx, I2C_SR1, I2C_FLAG_TXE))
	{
		...
		} else
		{
			...
			if(I2C_GetFlagStatus(pI2CHandle->pI2Cx, I2C_SR2, I2C_SR2_TRA) == SET)
			{
	...
	//6. Handle for interrupt generated by RXNE event
		...
		{
			//device mode is slave
			if(I2C_GetFlagStatus(pI2CHandle->pI2Cx, I2C_SR2, I2C_SR2_TRA) == RESET)
			{
				//device is receiving
 ...
 *
 * @brief				- Obtains value of a specific flag in the Statsu Register (only SR1 now)
 *
 * @param[in]			- Pointer to a I2C register
